        ...
        try{
            ...
            while (line!= null){
                for (byte element : line.getBytes(StandardCharsets.UTF_8)){
                    switch (element){
                        case 'T' : environment.add(new SolidSprite(imageTree, imageTreeHeight, imageTreeWidth,columnNumber*imageTreeWidth,
                                lineNumber*imageTreeHeight));
                                    break;
                        case ' ' : environment.add(new Sprite(imageGrass, imageGrassHeight,  imageGrassWidth,columnNumber*imageGrassWidth,
                                lineNumber*imageGrassHeight));
                            break;
                        case 'R' : environment.add(new SolidSprite(imageRock, imageRockHeight,  imageRockWidth,columnNumber*imageRockWidth,
                                lineNumber*imageRockHeight));
                            break;
                        case 'K' : environment.add(new DangerSolidSprite(imageTrap, imageTrapHeight,  imageTrapWidth,columnNumber*imageTrapWidth,
